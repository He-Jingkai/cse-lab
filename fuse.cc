...
void
fuseserver_symlink(fuse_req_t req,  const char *linkTo, fuse_ino_t parent, const char *name)
// fuseserver_symlink(fuse_req_t req,  const char *linkTo, fuse_ino_t parent, const char *name)
{
    fuse_entry_param e;
    int ret;
    ...
    chfs_client::inum inum;
    ret = chfs->mksymlink(parent,name,linkTo,inum);
...
fuseserver_readlink(fuse_req_t req, fuse_ino_t inum)
{
    std::string buf_out;
    chfs->readlink(inum,buf_out);
    fuse_reply_readlink(req,buf_out.c_str());
}
