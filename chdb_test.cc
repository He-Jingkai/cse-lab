...
TEST_CASE(part1, simple_tx_abort, "Simple transaction of abort") {
  ...
  int write_val = 1024;
  {
    ...
    ASSERT(db_client.tx_can_commit() == chdb_protocol::prepare_not_ok,
  ...
  // Check transaction result
  {
    ...
    ASSERT(db_client.tx_can_commit() == chdb_protocol::prepare_ok,
...
TEST_CASE(part1, random_tx_abort, "Random choose shard to abort") {
  ...
  bool commit_succ = chdb_protocol::prepare_ok;
  {
    ...
    ASSERT(commit_succ == db_client.tx_can_commit(),
           "Transaction result not match");
  }
}
...
TEST_CASE(part1, shard_dispatch_test_target_one, "Test assigned dispatch") {
  ...
  for (int idx = 0; idx < shard_num; ++idx) {
    ...
    ASSERT(sz == expected_distri[idx],
           "Shard dispatch distribution into one target bad");
  }
}

TEST_CASE(part1, shard_dispatch_test_in_static_rage,
          "Test assigned dispatch in static range") {
  int r;
...
TEST_CASE(part2, shard_client_replica, "Test shard client replication") {
  ...
  }
  {
    ...
    for (int key = 0; key < key_upper_bound; ++key) {
      r = db_client.get(key);
      ASSERT(r == write_val + key,
             "read bad result. Check the replication in shard client!");
    }
    ASSERT(db_client.tx_can_commit() == chdb_protocol::prepare_ok,
           "Transaction should commit");
  }
...
TEST_CASE(part2, shard_client_replica_with_put,
          "Test shard client shuffle replica before put") {
...
TEST_CASE(part3, tx_lock_test_1, "Test concurrent transaction") {
  ...
  for (int i = 0; i < 16; ++i) {
    tx_list.push_back(
        new std::thread([&store]() { tx_put_and_check(&store); }));
  }
...
#if BIG_LOCK
  ASSERT(false,
         "(Bonus testcase) Don't use big lock! Please disable BIG_LOCK in "
         "common.h");
#endif
}
