...
std::ofstream& operator<<(std::ofstream &m, const list_command& cmd) {
    m << cmd.value;
    // char buf[5];
    // cmd.serialize(buf, 4);
    // buf[4] = '\0';
...
std::ifstream& operator>>(std::ifstream &u, list_command& cmd) {
    u >> cmd.value;
    // char buf[5];
    // u>>buf;
    // cmd.deserialize(buf, 4);
    return u;
}
...
void list_state_machine::apply_snapshot(const std::vector<char>& snapshot) {
    ...
    // std::cout<<"snapshot stored success now store size: "<<store.size()<<"snapshot size"<<snapshot.size()<<std::endl; 
}
