...
class view_server {
 public:
  ...
  std::mutex lock_;
  int tx_holdLock = -1;
  view_server(const int base_port, shard_dispatch dispatch,
              const int num_raft_nodes = 3)
      : dispatch(dispatch), node(new rpc_node(base_port)) {
#if RAFT_GROUP
   ...
   * */
  int shard_num() const { return this->node->rpc_clients.size(); }

  /**
   * Dispatch the request to specific shard client(s)
   * Sync return when use normal view server (single node)
   * Async return when use raft group vie server, since the command log should
   * be distributed first.
   * */
  int execute(unsigned int query_key, unsigned int proc,
...
class chdb {
 public:
  chdb(const int shard_num, const int cluster_port,
       shard_dispatch dispatch = default_dispatch)
      : max_tx_id(0), vserver(new view_server(cluster_port, dispatch)) {
  ...
  bool acquireLock(int key, int tx_id) {
    lockMap_lock.lock();
    LockItem &lock_item = lockMap[key];
  ...
  bool releaseLock(int key, int tx_id) {
    lockMap_lock.lock();
    LockItem &lock_item = lockMap[key];
  ...
  std::map<int, LockItem> lockMap;
  std::mutex lockMap_lock;
