...
class tx_region {
 public:
  tx_region(chdb *db) : db(db), tx_id(db->next_tx_id()) { this->tx_begin(); }

  ~tx_region() {
    if (this->tx_can_commit() == chdb_protocol::prepare_ok)
      this->tx_commit();
    else
      this->tx_abort();
  }
   ...
   *
   * Note!: The changes in a the transaction region cannot be viewed by other
   * regions until it commits. Meanwhile, those changes can be **seen** in self
   * region.
   * */
